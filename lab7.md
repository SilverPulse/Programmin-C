...
typedef struct message {
    char content[MAX_MSG_LEN];    // содержимое сообщения (до 128 символов)
    char client_name[32];         // имя отправителя
} message;

typedef struct queue {
    message messages[MAX_QUEUE_SIZE];  // буфер сообщений (размер = 10)
    int front, rear, size;             // указатели на начало, конец и размер
